...
from .tensor_dataset_jax import TensorDataset
from ..transforms.data_processors_jax import DefaultDataProcessor
from ..transforms.normalizers_jax import UnitGaussianNormalizer
...
class PTDataset:
    ...
    def __init__(
        ...
        x_train = jnp.array(data["x"].numpy(), dtype=jnp.float32)
        if channels_squeezed:
            x_train = jnp.expand_dims(x_train, axis=channel_dim)
        ...
        y_train = jnp.array(data["y"].numpy())
        if channels_squeezed:
            y_train = jnp.expand_dims(y_train, axis=channel_dim)
        ...
        for res, n_test in zip(test_resolutions, n_tests):
            ...
            x_test = jnp.array(data["x"].numpy(), dtype=jnp.float32)
            if channels_squeezed:
                x_test = jnp.expand_dims(x_test, axis=channel_dim)
            # optionally subsample along data indices
            ...
            y_test = jnp.array(data["y"].numpy())
            if channels_squeezed:
                y_test = jnp.expand_dims(y_test, axis=channel_dim)
