...
class DataProcessor(nn.Module, metaclass=ABCMeta):
    ...
    def setup(self):
        ...
        - to(device): load necessary information to device, in keeping
            with JAX convention (JAX handles device placement automatically)
...
class DefaultDataProcessor(DataProcessor):
    ...
    in_normalizer: object = None
    out_normalizer: object = None
    ...
    def preprocess(self, data_dict, batched=True):
        ...
        """
        x = jnp.asarray(data_dict["x"])
        y = jnp.asarray(data_dict["y"])
    ...
    def postprocess(self, output, data_dict, training: bool = True):
    ...
    def __call__(self, data_dict, training: bool = True):
        ...
        output, data_dict = self.postprocess(output, data_dict, training=training)
        return output, data_dict


class IncrementalDataProcessor(nn.Module):
    def __init__(
        self,
        in_normalizer=None,
        out_normalizer=None,
    ...
    def regularize_input_res(self, x, y):
        ...
        for idx in self.dataset_indices:
            indexes = jnp.arange(0, x.shape[idx], self.current_sub)
            x = jnp.take(x, indexes, axis=idx)
            y = jnp.take(y, indexes, axis=idx)
        return x, y
...
class MGPatchingDataProcessor(DataProcessor):
    ...
    def preprocess(self, data_dict, batched=True):
        ...
        """
        data_dict = {
            k: jnp.asarray(v) for k, v in data_dict.items() if isinstance(v, jnp.ndarray)
        }
    ...
    def postprocess(self, out, data_dict, training: bool = True):
        """
        ...
        """
        y = data_dict["y"]
        out, y = self.patcher.unpatch(out, y, evaluation=not training)
