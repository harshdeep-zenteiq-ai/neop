...
class GINO(BaseModel):
    ...
    def __init__(
        ...
        self.fno_domain_padding = fno_domain_padding 
        self.out_gno_tanh = out_gno_tanh

        # Storage for neighbor search results (for logging)
        # self.last_gno_in_neighbors = None
        # self.last_gno_in_query = None
        # self.last_gno_in_data = None
        # self.last_gno_out_neighbors = None
        # self.last_gno_out_query = None
        # self.last_gno_out_data = None
    ...
    def forward(self, input_geom, latent_queries, output_queries,
        ...
        # Capture neighbor search results from input GNO for logging
        # self.last_gno_in_neighbors = self.gno_in.neighbor_search(
        #     data=input_geom,
        #     queries=latent_queries.view((-1, latent_queries.shape[-1])),
        #     radius=self.gno_in.radius
        # )
        # self.last_gno_in_query = latent_queries.view((-1, latent_queries.shape[-1]))
        # self.last_gno_in_data = input_geom

        # Pass through input GNOBlock
        # print("GINO forward pass: input GNO...")
        in_p = self.gno_in(
        ...
        if isinstance(output_queries, dict):
            ...
            for key, out_p in output_queries.items():
                ...
                # Capture neighbor search results from output GNO for logging
                # self.last_gno_out_neighbors = self.gno_out.neighbor_search(
                #     data=latent_queries.reshape((-1, latent_queries.shape[-1])),
                #     queries=out_p,
                #     radius=self.gno_out.radius
                # )
                # self.last_gno_out_query = out_p
                # self.last_gno_out_data = latent_queries.reshape((-1, latent_queries.shape[-1]))
                
                # print("GINO forward pass: output GNO for query set {key}...".format(key=key))
            ...
            #     radius=self.gno_out.radius
            # )
            # self.last_gno_out_query = output_queries
            # self.last_gno_out_data = latent_queries.reshape((-1, latent_queries.shape[-1]))

            # latent queries is of shape (d_1 x d_2 x... d_n x n), reshape to n_out x n
            # print("GINO forward pass: output GNO for query set...")
            out = self.gno_out(
                y=latent_queries.reshape((-1, latent_queries.shape[-1])),
                x=output_queries,
                f_y=latent_embed,
    ...
    def precompute_neighbors(self, dataset, output_n_points=None, log_dir=None, split="train"):
        ...
        Runs neighbor search for gno_in (per sample) and gno_out (per sample)
        before training begins, so no neighbor search is repeated during training.

        Results are stored directly into dataset.data_list[i] as:
        ...
        output_n_points : int, optional
            Number of output query points (vertices used as output_queries).
            Must match the truncation applied in your data processor.
            If None, all vertices are used.
        log_dir : str or Path, optional
            Directory to write per-sample neighbor count log files.
            Files are named gno_in_{split}{i:03d}.txt and gno_out_{split}{i:03d}.txt.
            If None, no files are written.
        split : str, optional
            Label for the dataset split, e.g. "train" or "test". Default: "train".
        """
        ...
        # Precompute gno_in and gno_out neighbors per sample.
        # gno_in: for each grid point, which mesh vertices are nearby?
        #         data=vertices (per-car), queries=grid (fixed)
        # gno_out: for each output surface point, which grid points are nearby?
        #          data=grid (fixed), queries=output vertices (per-car)
        log_file = None
        if log_dir is not None:
        ...
        try:
            for i, sample in enumerate(dataset.data_list):
                ...
                sample["neighbors_out"] = self.gno_out.neighbor_search(
