...
class NeighborSearch(nn.Module):
    ...
    def __init__(self, use_open3d=True, return_norm=False):
        ...
        if use_open3d and open3d_built:  # slightly faster, works on GPU in 3d only
            ...
            # print("Using native PyTorch implementation for neighbor search")
    ...
    def forward(self, data, queries, radius):
        ...
        # print('Neighbor search found {} neighbors.'.format(return_dict["neighbors_index"].shape))
        return return_dict
...
def native_neighbor_search(
    ...
    # print('return norms: {}'.format(return_norm))
    # sys.exit(0) 
    if return_norm:
